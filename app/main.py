...
class JobSubmitRequest(BaseModel):
    ...
    # Riboswitch design fields (SMART pipeline)
    structure_on: Optional[str] = None
    structure_off: Optional[str] = None
    population: int = 100
    generations: int = 50
...
async def _create_job_impl(body: JobSubmitRequest) -> Dict[str, Any]:
    ...
    if body.structure_on is not None or body.structure_off is not None:
        s_on = (body.structure_on or "").strip()
        s_off = (body.structure_off or "").strip()
        if not s_on or not s_off:
            raise HTTPException(
                status_code=400,
                detail="Both 'structure_on' and 'structure_off' are required for riboswitch design",
            )
        if len(s_on) != len(s_off):
            raise HTTPException(
                status_code=400,
                detail=(
                    f"'structure_on' and 'structure_off' must be the same length "
                    f"(got {len(s_on)} and {len(s_off)})"
                ),
            )
        invalid_on = set(s_on) - _DOT_BRACKET
        invalid_off = set(s_off) - _DOT_BRACKET
        if invalid_on or invalid_off:
            bad = (invalid_on | invalid_off)
            raise HTTPException(
                status_code=400,
                detail=f"Structures must contain only '.', '(', ')' — found: {sorted(bad)}",
            )
        ga_params = {
            "structure_on": s_on,
            "structure_off": s_off,
            ...
            "bp_distance_obj": body.bp_distance_obj,
        }
        if body.seed is not None:
            ga_params["seed"] = body.seed
        job_id = await insert_task(
            {
                "type": "riboswitch_design",
                "input_type": "riboswitch",
                "input_value": json.dumps(ga_params),
                "status": "queued",
                "created_at": datetime.now(),
            }
        )
        logger.info(f"Riboswitch job created: {job_id}")
        return {
            "job_id": job_id,
            "status": "queued",
            "input_type": "riboswitch",
            "message": "Riboswitch design job queued — CoordinatorAgent will pick it up within 5s",
        }
...
# ---------------------------------------------------------------------------
# Protein 3-D viewer — serves the OutputAgent HTML report
# ---------------------------------------------------------------------------

_OUTPUTS_DIR = os.path.join(os.path.dirname(__file__), "..", "data", "outputs")


@app.get("/jobs/{job_id}/viewer", response_class=HTMLResponse)
async def get_job_viewer(job_id: int) -> str:
    """Serve the 3Dmol.js HTML report generated by OutputAgent for a protein job."""
    _require_db()
    job = await get_task_by_id(job_id)
    if not job:
        raise HTTPException(status_code=404, detail=f"Job {job_id} not found")

    # Derive accession: for accession-type jobs use input_value directly;
    # for fasta jobs the accession may be stored in the result.
    accession = job.get("input_value", "")
    html_path = os.path.normpath(os.path.join(_OUTPUTS_DIR, f"{accession}.html"))

    if not os.path.isfile(html_path):
        # Fallback: try finding via result record
        result = await get_result_by_task_id(job_id)
        if result:
            accession = result.get("accession", accession)
            html_path = os.path.normpath(os.path.join(_OUTPUTS_DIR, f"{accession}.html"))

    if not os.path.isfile(html_path):
        raise HTTPException(status_code=404, detail="3D viewer report not found — job may still be processing")
...
# ---------------------------------------------------------------------------
# Riboswitch visualisation proxy
# ---------------------------------------------------------------------------

_RIBOSWITCH_URL = os.getenv("RIBOSWITCH_SERVICE_URL", "http://riboswitch-service:8001")


@app.post("/visualize")
async def visualize(body: Dict[str, Any]) -> Dict[str, Any]:
    """
    Proxy POST /visualize to the riboswitch service and return base64 PNG images.
    Expected body: { structure_on, structure_off, sequence?, graph_mode?, algorithm? }
    Returns: { arc, circos, varna_on, varna_off } — all base64 PNG strings (varna_* may be null).
    """
    def _call():
        return _requests.post(f"{_RIBOSWITCH_URL}/visualize", json=body, timeout=120)

    try:
        loop = asyncio.get_event_loop()
        resp = await loop.run_in_executor(None, _call)
    except _requests.ConnectionError:
        raise HTTPException(status_code=503, detail="Riboswitch service unreachable")
    except _requests.Timeout:
        raise HTTPException(status_code=504, detail="Riboswitch service timed out")
